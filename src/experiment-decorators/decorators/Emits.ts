 ...
 * }
 */
export const Emits = <E extends ChangeEvent, D = object>(event: { eventType: string; make: ChangeEventFactory<E, D> }) => {
  type StateChangeMethodDescriptor = TypedPropertyDescriptor<(data: D) => void> | TypedPropertyDescriptor<() => void>
  ...
  return (entity: object, methodName: string, descriptor: StateChangeMethodDescriptor) => {
    ...
    descriptor.value = function (this: EventSourcedEntity, data: D) {
      ...
      this.aggregate.addChangeEvent(
        event.make(Uuid.createV4, {
          entityId: this.id,
