...
describe('AggregateContainer', () => {
  ...
  describe('loadFromVersion', () => {
    it('multiple events', async () => {
      ...
      const event: ChangeEvent = ChangeEventBuilder.make().withAggregateRootId(id).to()
  ...
  describe('observe', () => {
    ...
    beforeEach(() => {
      const id = Uuid.createV4()
      event = ChangeEventBuilder.make().withAggregateRootId(id).withCorrelation(correlationId).to()
    })

    it('change', async () => {
      aggregate.loadFromHistory([{ version: 0, event }])

      const nextEvent = ChangeEventBuilder.make().withAggregateRootId(event.aggregateRootId).to()
    ...
    it('correlated change', async () => {
      ...
      assertThat(aggregate.uncommittedChanges()).is([{ version: 1, event: correlatedEvent }])
