...
import { Person } from '..'
import * as personEvents from "../events/personEvents"
...
describe('Device', () => {
  describe('GenericAggregateRoot', () => {
    describe('Event Sourceing Basics', () => {
      it('Uninitilaised', () => {
        const aggregate = new AggregateContainer<Person>((parent) => new Person(parent))
      ...
      it('Create New Device', () => {
        ...
        const aggregate = new AggregateContainer<Person>(
          (parent, id) => new Person(parent, id ), 
          id
        )
        const events = aggregate.uncommittedChanges()
        assertThat(events).is([
          makeEntityEventMatcher(new personEvents.PersonCreatedEvent(id, id), 0)
        ])
        assertThat(aggregate.changeVersion).is(UNINITIALISED_AGGREGATE_VERSION)
      })
      ...
      it('Load from Histoy', () => {
        ...
        const id = Uuid.createV4()
        const history = [{ event: new personEvents.PersonCreatedEvent(id, id), version: 0 }]
    ...
    //     const personId = Uuid.createV4()
    //     const aggregate = new AggregateContainer<Person>((parent, id) => new Person(parent, id), deviceId) //+1
    ...
    //     const hydratedAggregate = new AggregateContainer<Person>((parent) => new Person(parent)) //+1
